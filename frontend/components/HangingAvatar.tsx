...
export function HangingAvatar({ emotion, character = "spiderman", size = 220, isListening, isSpeaking }: Props) {
  ...
  if (!mounted) return (
    <div style={{ width: size, minHeight: size + 160 }} className="relative flex flex-col items-center">
      <div style={{ marginTop: 120 }}>
        <Avatar emotion={emotion} character={character} size={size} />
      </div>
    </div>
  );
      ...
      <AnimatePresence>
        {webPulse && (
          <motion.div
            ...
            style={{
              top: -20, left: "50%", transform: "translateX(-50%)", zIndex: 3,
        ...
        >
          <AnimatePresence>
            {!isHidden && (
              ...
              >
                <Avatar emotion={eyesBlink ? "neutral" : emotion} character={character} size={size} />
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
