...
export function CallScreen({ callee, isIncoming = false, onEnd }: CallScreenProps) {
  const displayName = callee === "mom" ? "Mom 💜" : callee === "dad" ? "Dad 💙" : `${callee} 💜`;
  const avatar      = callee === "mom" ? "👩" : callee === "dad" ? "👨" : "📞";
  ...
  useEffect(() => {
    if (!isIncoming && status === "ready" && !ranRef.current) {
      ranRef.current = true;
      ringParent();
    }
  ...
  // incoming ring (from the parent) is answered immediately.
  useEffect(() => {
    if (status === "incoming") {
      acceptCall();
    }
